...
def setup_train_data(colorConv, orient, pix_per_cell, cell_per_block, hog_channel):
    ...
    """
    cars = []
    images = glob.glob('/home/frostbyte/Desktop/CNU/parking-tracker/Automatic-Parking-Management/Khare_training_01/vehicles/**/*.png', recursive=True)
    for image in images:
        cars.append(image)
    
    images = glob.glob('/home/frostbyte/Desktop/CNU/parking-tracker/Automatic-Parking-Management/Khare_training_01/non-vehicles/**/*.png', recursive=True)
    notcars = []
    for image in images:
...
if __name__ == "__main__":
	...
	# '''
	# recent_heatmaps = deque(maxlen=10)
	# project_video_res = 'parking_garage.mp4'
	# clip1 = VideoFileClip("parking_garage.mp4")
	# project_video_clip = clip1.fl_image(process_img)
	# project_video_clip.write_videofile(project_video_res, audio=False)

	# recent_heatmaps = deque(maxlen=10)
	# project_video_res = 'parking_garage_2.mp4'
	# clip1 = VideoFileClip("parking_garage_2.mp4")
	# project_video_clip = clip1.fl_image(process_img)
	# project_video_clip.write_videofile(project_video_res, audio=False)