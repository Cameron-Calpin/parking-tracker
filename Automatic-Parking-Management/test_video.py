...
#cnx = mysql.connector.connect(user='parking', password='Parking!123',
#        host='10.0.0.50',
...
cascade_src = 'Khare_training_01/data/cascade.xml'
...
dict =  {
        ...
        'park_laplacian_th': 2.8, #2.8
        'park_sec_to_wait': 0.2, # 4   wait time for changing the status of a region
...
def run_classifier(img, id):
    ...
    cars = car_cascade.detectMultiScale(img, 1.2, 2) # 1.1, 1
    if cars == ():
        return False
    else:
...
# Grab the parking id's from the dictionary
id_values = []
id_string = ""
for x in parking_data:
    for keys, values in x.items():
        if keys == 'id':
            id_values.append(values)

# Create dictionary for Database usage
id_status_dict = {}
for i in id_values:
    id_status_dict[i] = 0
...
#for y in id_status_dict:
#    id_string = id_string + "({0}, 0),".format(str(y))
#id_string = id_string[:-1]

# print(id_string)
#drp_tbl = "DROP TABLE indicator"
#sql = "CREATE TABLE IF NOT EXISTS parking_tracker.indicator (park_id CHAR(20) DEFAULT '' NOT NULL, park_status INT UNSIGNED DEFAULT '0' NOT NULL, PRIMARY KEY(park_id))"
#val = "INSERT IGNORE INTO `indicator` (`park_id`, `park_status`) VALUES {0};".format(id_string)

#mycursor.execute(drp_tbl)
...
#    # Append data to json file (time, park_id, park_status)
...
#        data = {}
#        data['example'] = []
#        data['example'].append(timestmp)
#        data['example'].append(id_status_dict)

#        with open('data.txt', 'a+') as outfile:  
#            json.dump(data, outfile, default=str)
#            outfile.write('\n')

#        # s.enter(60, 1, log_data, (sc,))
...
#    s.run()
    ...
    if dict['text_overlay']:
        ...
        cv2.putText(frame_out, str_on_frame, (5,30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2, cv2.LINE_AA)
        cv2.putText(frame_out,global_str + str(round(change_pos,2)) + 'sec', (5, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 0), 2, cv2.LINE_AA)
    ...
    if dict['parking_overlay']:
        for ind, park in enumerate(parking_data):
            ...
            if parking_status[ind]:
                # print("id: {0}, ind: {1}".format(park['id'], parking_status[ind]))

#                if id_status_dict[park['id']] == 0:
#                   id_status_dict[park['id']] = 1

#                   mycursor = cnx.cursor()
#                  
#                   updt_table = "UPDATE `indicator` SET `park_status` = 1 WHERE `park_id` = {0};".format(park['id'])

#                   mycursor.execute(updt_table)
...
#                if id_status_dict[park['id']] == 1:
#                   id_status_dict[park['id']] = 0

#                   mycursor = cnx.cursor()

#                   updt_table = "UPDATE `indicator` SET `park_status` = 0 WHERE `park_id` = {0};".format(park['id'])
